...
export class UsersClient {
     ...
     * @return OK
     */
    update(id: number, body: UpdateUserRequest | undefined): Promise<void> {
        let url_ = this.baseUrl + "/api/Users/{id}";
        if (id === undefined || id === null)
            throw new globalThis.Error("The parameter 'id' must be defined.");
        url_ = url_.replace("{id}", encodeURIComponent("" + id));
     ...
     * @return OK
     */
    getByTenant(tenantId: number): Promise<User[]> {
        let url_ = this.baseUrl + "/api/Users/by-tenant/{tenantId}";
    ...
    protected processGetByTenant(response: Response): Promise<User[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
     ...
     * @return OK
     */
    create(body: CreateUserRequest | undefined): Promise<void> {
